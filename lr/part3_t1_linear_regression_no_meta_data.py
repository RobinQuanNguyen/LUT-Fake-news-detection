"""
PART 3 - TF-IDF + SGDClassifier (Full Dataset)
...
RANDOM_SEED = 42
BATCH_SIZE = 50_000   # Incremental batch size
...
def plot_confusion_matrix(cm, class_names, save_path):
    ...
    ax.set_title('Confusion Matrix - Fake News Detection (Full Dataset)', fontsize=14)
...
def save_results(model, vectorizer, val_metrics, test_metrics, train_size, val_size, test_size):
    ...
    summary = {
        "model": model_name,
        "features": "processed_text (TF-IDF - Full Dataset)",
    ...
    txt_output.append(f"{model_name} (FULL DATASET) - RESULTS REPORT")
    txt_output.append("=" * 70)
...
def main():
    print_log("=" * 70)
    print_log("PART 3 - TF-IDF + SGDClassifier (Full Dataset)")
    ...
    # Load data
    train_df = load_prepare(DATA_DIR / "train.csv", "train")
    val_df = load_prepare(DATA_DIR / "validate.csv", "validate")
    test_df = load_prepare(DATA_DIR / "test.csv", "test")
    ...
    # Save
    summary = save_results(model, vectorizer, val_metrics, test_metrics,
