"""
PART 4 - DistilBERT Fine-tuning for Fake News Detection (Text Only)
Uses pretrained DistilBERT for sequence classification.
"""
...
# ========== CONFIGURATION ==========
DATA_DIR = Path("data")
OUTPUT_DIR = Path("outputs/part4_text")
...
def save_results(results, train_time, label_encoder):
    ...
    # Classification report (cần test_labels thực tế)
    ...
    txt_output.append("DISTILBERT - RESULTS REPORT (Text Only)")
    txt_output.append("=" * 60)
    ...
    txt_output.append(f"Features      : processed_text")
    txt_output.append("")
...
def main():
    print_log("=" * 60)
    print_log("PART 4 - DistilBERT Fine-tuning (Text Only)")
    print_log("=" * 60)
    ...
    print_log("\n" + "=" * 60)
    print_log("FINAL RESULTS - DistilBERT (Text Only)")
    print_log("=" * 60)
