...
TRAIN_PATH = Path("data/metadata/train_meta.csv")
VAL_PATH = Path("data/metadata/validate_meta.csv")
TEST_PATH = Path("data/metadata/test_meta.csv")
LIAR_TEST_PATH = Path("data/evaluate/test.tsv")

MODEL_DIR = Path("models")
PREPROCESSOR_CACHE_PATH = MODEL_DIR / "logreg_metadata_preprocessor.joblib"
SKLEARN_MODEL_CACHE_PATH = MODEL_DIR / "logreg_metadata_sklearn.joblib"
TORCH_MODEL_CACHE_PATH = MODEL_DIR / "logreg_metadata_torch.pt"
...
def load_liar_test(path):
    ...
    liar_df = pd.DataFrame(
        {
            "processed_text": df["statement"].astype(str),
            # LIAR does not have a direct publisher-domain column.
            "domain": "",
            # Use subject and speaker as the closest metadata proxies.
            "title": df["subject"].fillna("").astype(str),
            "authors": df["speaker"].fillna("").astype(str),
            "label": df["label"],
        }
    )
...
def load_or_fit_preprocessor(X_train):
...
def main():
    ...
    train_df = load_metadata_split(TRAIN_PATH)
    val_df = load_metadata_split(VAL_PATH)
    test_df = load_metadata_split(TEST_PATH)
    liar_df = load_liar_test(LIAR_TEST_PATH)

    X_train_df = train_df[TEXT_COLUMNS]
    y_train = train_df["label"].to_numpy()

    X_val_df = val_df[TEXT_COLUMNS]
    y_val = val_df["label"].to_numpy()

    X_test_df = test_df[TEXT_COLUMNS]
    y_test = test_df["label"].to_numpy()

    X_liar_df = liar_df[TEXT_COLUMNS]
    y_liar = liar_df["label"].to_numpy()

    preprocessor = load_or_fit_preprocessor(X_train_df)

    print("\nTransforming datasets into TF-IDF features...")
    X_train = preprocessor.transform(X_train_df)
    X_val = preprocessor.transform(X_val_df)
    X_test = preprocessor.transform(X_test_df)
    X_liar = preprocessor.transform(X_liar_df)
